...
// playtest-crew SDK integration: the one required call (sdk/README.md step
// 2). `window.game` is created by index.html's own inline script, which
// runs after this file, but that's fine -- this closure only reads it when
// the harness actually calls it, well after the page has fully loaded.
// Per-cell state lives as custom properties on the `.cell` DOM elements
// (isMasked/isBomb/isFlagged/mine_count), not in a central array, so this
// reads them off the live DOM rather than off any Game instance field.
window.PTC.exposeState(function () {
  var cells = Array.prototype.map.call(document.getElementsByClassName('cell'), function (c) {
    return {
      isMasked: !!c.isMasked,
      isBomb: !!c.isBomb,
      isFlagged: !!c.isFlagged,
      mineCount: c.mine_count === undefined ? null : c.mine_count
    }
  })
  return {
    cells: cells,
    result: window.game.result || null,
    moves: window.game.moves || 0,
    cols: window.game.cols,
    rows: window.game.rows,
    bombs: window.game.number_of_bombs
  }
})
